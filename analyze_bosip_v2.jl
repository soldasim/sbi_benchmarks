...
if !isnothing(X_duff_std)
    ...
    q = quantile(alpha_vals, [0.0, 0.25, 0.5, 0.75, 1.0])
    println("  Alpha quantiles [min, Q1, median, Q3, max]: [$(round(q[1];digits=6)), $(round(q[2];digits=6)), $(round(q[3];digits=6)), $(round(q[4];digits=6)), $(round(q[5];digits=6))]")
end
