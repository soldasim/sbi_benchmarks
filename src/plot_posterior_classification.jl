"""
Posterior classification scatter plot — mean log-TV score variant.

Colour axis: median over runs of the per-run mean log(TV), i.e. the arithmetic
mean of log(TV(t)) over all valid iterations.

Data is read dynamically from:
  plots/classify_posteriors.csv   — h_rel, n_modes per problem
  plots/acq_scores.csv            — eiv_median, maxvar_median per problem

Run from ~/repos/bosip_benchmarks/ via include() in a live Julia session.
Results saved to plots/posterior_classification.{png,pdf}.
"""
...
const D_MAX_PC       = 1.0
const DRAW_THRESH_PC = log(1.20)   # ≈ 0.182 — 20% better to count as a win

function margin_color_pc(diff::Real)
    d = clamp(Float64(diff), -D_MAX_PC, D_MAX_PC)
    t = if d <= -DRAW_THRESH_PC
            0.25 * (d + D_MAX_PC) / (D_MAX_PC - DRAW_THRESH_PC)
        elseif d <= DRAW_THRESH_PC
            0.5 + 0.1 * d / DRAW_THRESH_PC
        else
            0.75 + 0.25 * (d - DRAW_THRESH_PC) / (D_MAX_PC - DRAW_THRESH_PC)
        end
    c = cgrad(:coolwarm)[t]
    f = 0.80f0
...
function _read_csv_cols_pc(path, cols)
...
_scr_rows_pc = _read_csv_cols_pc("plots/acq_scores.csv",
                                  ["problem", "eiv_median", "maxvar_median"])
_scr_dict_pc = Dict(r[1] => parse(Float64, r[2]) - parse(Float64, r[3]) for r in _scr_rows_pc)

# classification: problem → (H_rel, n_modes)
_cls_rows_pc = _read_csv_cols_pc("plots/classify_posteriors.csv",
                                  ["problem", "H_rel", "n_modes"])

const _BIP_NAMES_PC = Set(["ABProblem", "SimpleProblem", "BananaProblem", "BimodalProblem",
                            "SIRProblem", "DuffingProblem", "DiffusionProblem10"])

function _display_label_pc(prob)
...
const PCLASS_DATA_PC = [
    (_display_label_pc(r[1]),
     parse(Float64, r[2]),
     parse(Int,     r[3]),
     _scr_dict_pc[r[1]],
     r[1] in _BIP_NAMES_PC ? :bip : :opt)
    for r in _cls_rows_pc if haskey(_scr_dict_pc, r[1])
]

## ── Automatic label placement via force-directed repulsion ───────────────────

function _auto_label_offsets_pc(items;
    xrange    = (0.0, 1.0),
...
_quad_d2s_pc(h, m; yrange = (log10(0.60), log10(50.0)), fig_w = 620.0, fig_h = 380.0) =
    ((h - 0.0) / 1.0 * fig_w, (log10(m) - yrange[1]) / (yrange[2] - yrange[1]) * fig_h)
_QUAD_OBSTACLES_PC = [
    (px + (ha == :left ? 25.0 : -25.0), py + (va == :bottom ? 12.0 : -12.0))
    for (hc, mc, ha, va) in _QUAD_ANCHORS_PC
    for (px, py) in (_quad_d2s_pc(hc, mc),)
]

_lox_pc, _loy_pc = _auto_label_offsets_pc(PCLASS_DATA_PC; obstacles = _QUAD_OBSTACLES_PC)

## ── Figure ───────────────────────────────────────────────────────────────────

mkpath("plots")

@assert all(m <= 50 for (_, _, m, _, _) in PCLASS_DATA_PC) "A problem exceeds the y-axis limit of 50 modes"

fig_pc = Figure(; size = (800, 460))

ax_pc = Axis(fig_pc[1, 1];
...
## Threshold lines
vlines!(ax_pc, [0.50]; color = (:black, 0.28), linestyle = :dash, linewidth = 1.0)
hlines!(ax_pc, [1.5];  color = (:black, 0.28), linestyle = :dash, linewidth = 1.0)
...
for (hc, ha, mc, va, lbl) in [
    ...
    text!(ax_pc, hc, mc; text=lbl, color=(RGBf(0.05, 0.20, 0.05), 0.5), fontsize=10,
          align=(ha, va), font=:italic)
end

## Scatter + labels
for (i, (lbl, h, m, d, grp)) in enumerate(PCLASS_DATA_PC)
    col = margin_color_pc(d)
    mf  = Float64(m)
    ...
    text!(ax_pc, h, mf;
        ...
        align    = (:center, :center),
        offset   = Point2f(_lox_pc[i], _loy_pc[i]),
    )
end

## Colorbar
_cmap_pc = let n = 256
    cgrad([margin_color_pc(-D_MAX_PC + 2*D_MAX_PC*(i-1)/(n-1)) for i in 1:n])
end
Colorbar(fig_pc[1, 2];
    colormap   = _cmap_pc,
    limits     = (-D_MAX_PC, D_MAX_PC),
    ticks      = ([-D_MAX_PC, -DRAW_THRESH_PC, 0, DRAW_THRESH_PC, D_MAX_PC],
                  ["< -$(Int(D_MAX_PC))\n(EIV wins)", "-0.2", "0", "+0.2",
                   "> +$(Int(D_MAX_PC))\n(MaxVar wins)"]),
    label      = "Score margin: EIV − MaxVar  (median mean log-TV)",
    ticklabelsize = 11,
    labelsize  = 12,
    labelpadding = -50,
    width      = 14,
...
colsize!(fig_pc.layout, 1, Relative(0.85))

## Save
save("plots/posterior_classification.png", fig_pc; px_per_unit = 3)
save("plots/posterior_classification.pdf", fig_pc)
@info "Saved → plots/posterior_classification.{png,pdf}"
