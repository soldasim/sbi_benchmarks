...
# Simple 2D GP-sampled objective problem
struct GP2DProblem <: AbstractProblem
    f::Function
    bounds::Tuple{NTuple{2, Float64}, NTuple{2, Float64}}
end

function sample_gp_problem_2d()
    bounds = ((0.0, 0.0), (1.0, 1.0))

    # Sample a 2D objective from a GP prior
    f_sample = sample_gp_function_2d(; bounds)
    problem = GP2DProblem(f_sample, bounds)

    return problem
end

# --- Problem API ---
simulator(p::GP2DProblem) = x -> [p.f(x)]
domain(p::GP2DProblem) = Domain(; bounds = ([p.bounds[1]...], [p.bounds[2]...]))
likelihood(::GP2DProblem) = NormalLikelihood(; z_obs = [0.0], std_obs = [1.0])
prior_mean(::GP2DProblem) = [0.0]
x_prior(p::GP2DProblem) = product_distribution([Uniform(p.bounds[1][i], p.bounds[2][i]) for i in 1:2])
est_amplitude(::GP2DProblem) = [1.0]
est_noise_std(::GP2DProblem) = nothing
true_f(p::GP2DProblem) = x -> [p.f(x)]
get_lengthscale_priors(::GP2DProblem) = fill(product_distribution([LogNormal(log(0.2), 0.5) for _ in 1:2]), 1)
get_amplitude_priors(::GP2DProblem) = fill(LogNormal(log(1.0), 0.5), 1)
get_noise_std_priors(::GP2DProblem) = fill(LogNormal(log(0.05), 0.5), 1)
...
function tnp_test_2d()
    ...
    Z_mean_gp = Matrix{Float64}(undef, n_test, n_test)
    Z_std_gp = Matrix{Float64}(undef, n_test, n_test)
    Z_mean_tnp = Matrix{Float64}(undef, n_test, n_test)
    ...
    # Column 1: True
    ax1 = Axis(fig[1, 1]; title = "True function (GP sample)", xlabel = "x₁", ylabel = "x₂")
    ...
    ax2 = Axis(fig[2, 1]; title = "True posterior", xlabel = "x₁", ylabel = "x₂")
    hm2 = heatmap!(ax2, x1_test, x2_test, Z_post_true)
    scatter!(ax2, X_train[1,:], X_train[2,:]; color = :white, markersize = 6, strokewidth = 1, strokecolor = :black)
    Colorbar(fig[2, 2], hm2)

    # Column 2: GP
    ax3 = Axis(fig[1, 3]; title = "GP mean", xlabel = "x₁", ylabel = "x₂")
    hm3 = heatmap!(ax3, x1_test, x2_test, Z_mean_gp)
    scatter!(ax3, X_train[1,:], X_train[2,:]; color = :white, markersize = 6, strokewidth = 1, strokecolor = :black)
    Colorbar(fig[1, 4], hm3)

    ax4 = Axis(fig[2, 3]; title = "GP posterior mean", xlabel = "x₁", ylabel = "x₂")
    hm4 = heatmap!(ax4, x1_test, x2_test, Z_post_mean_gp)
    scatter!(ax4, X_train[1,:], X_train[2,:]; color = :white, markersize = 6, strokewidth = 1, strokecolor = :black)
    Colorbar(fig[2, 4], hm4)

    # Column 3: TNP
    ax5 = Axis(fig[1, 5]; title = "TNP mean", xlabel = "x₁", ylabel = "x₂")
    hm5 = heatmap!(ax5, x1_test, x2_test, Z_mean_tnp)
    scatter!(ax5, X_train[1,:], X_train[2,:]; color = :white, markersize = 6, strokewidth = 1, strokecolor = :black)
    Colorbar(fig[1, 6], hm5)

    ax6 = Axis(fig[2, 5]; title = "TNP posterior mean", xlabel = "x₁", ylabel = "x₂")
    hm6 = heatmap!(ax6, x1_test, x2_test, Z_post_mean_tnp)
    scatter!(ax6, X_train[1,:], X_train[2,:]; color = :white, markersize = 6, strokewidth = 1, strokecolor = :black)
    Colorbar(fig[2, 6], hm6)

    save("plots/test_2d.png", fig)
    fig
end
