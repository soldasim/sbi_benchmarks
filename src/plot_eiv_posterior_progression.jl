# 6-row × 5-column posterior progression plot: without/with proxy for SIR, Beale, Goldstein-Price.
# Rows (alternating without/with proxy):
#   1. SIR (no proxy)                 — data-bosip/SIRProblem,                   method: eiv
#   2. SIR (proxy)                    — data-bosip/ProxySIRProblem,              method: eiv  (no data → "no data")
#   3. Beale (no proxy)               — data-opt-functions/BealeProblem,          method: eiv
#   4. Beale (proxy)                  — data-opt-functions/BealeProxyProblem,     method: eiv
...
#   julia --project=src src/plot_eiv_posterior_progression.jl
...
function add_learned_panel!(fig_pos, problem::AbstractProblem, data_root::String,
    ...
    actual_n = min(n_points, n_avail)
    truncated = (actual_n < n_points)
    if truncated
        ax.title = "iter $(actual_n - INIT_DATA) (max)"
    end
    N = actual_n
...
rows = [
    (SIRProblem(),                 "data-bosip",        "eiv", 1, 103, "SIR\n(no proxy)"),
    (ProxySIRProblem(),            "data-bosip",        "eiv", 1, 103, "SIR\n(proxy)"),
    (BealeProblem(),               "data-opt-functions", "eiv", 1, 203, "Beale\n(no proxy)"),
    (BealeProxyProblem(),          "data-opt-functions", "eiv", 1, 203, "Beale\n(proxy)"),
    (GoldsteinPriceProblem(),      "data-opt-functions", "eiv", 1, 203, "Goldstein-Price\n(no proxy)"),
    (GoldsteinPriceProxyProblem(), "data-opt-functions", "eiv", 1, 203, "Goldstein-Price\n(proxy)"),
]
...
@info "Building 6×5 EIV posterior progression plot ..."
...
save(joinpath(PLOT_DIR, "eiv_posterior_progression.png"), fig)
save(joinpath(PLOT_DIR, "eiv_posterior_progression.pdf"), fig)
@info "Done. Saved to $(PLOT_DIR)/eiv_posterior_progression.png"
