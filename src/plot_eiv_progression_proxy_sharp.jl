# 24-row × 5-column EIV progression plot — sharp posteriors, proxy variants substituted.
...
const PROXY_PROBLEMS_EIV_SHARP = Set(["BealeProxyProblem_sharp", "GoldsteinPriceProxyProblem_sharp"])

all_problems_eiv_sharp = SharpProblem.([
    # Original 3
...
@assert length(all_problems_eiv_sharp) == 24

# ─── GP refit helpers ─────────────────────────────────────────────────────────

function make_bosip_eiv_sharp(problem, X, Y)
...
function refit_eiv_sharp!(bosip)
...
function add_eiv_learned_panel_sharp!(fig_pos, problem, run_name, run_idx, n_points; grid_size=60)
    ax = Axis(fig_pos;
    ...
    if get_name(problem) in PROXY_PROBLEMS_EIV_SHARP
        ...
        bosip = make_bosip_eiv_sharp(problem, X_full[:, 1:N], Y_full[:, 1:N])
        refit_eiv_sharp!(bosip)
...
function add_eiv_true_panel_sharp!(fig_pos, problem; grid_size=60)
    ax = Axis(fig_pos;
        xticklabelsvisible=false, yticklabelsvisible=false,
        xticksvisible=false, yticksvisible=false,
        leftspinevisible=false, rightspinevisible=false,
...
nrows, ncols = length(all_problems_eiv_sharp), length(col_labels)
cw, ch = 180, 180

@info "Building 24×5 EIV progression plot (sharp, proxy substituted) ..."
...
for (r, prob) in enumerate(all_problems_eiv_sharp)
    name = get_name(prob)
    @info "  Row $r: $name ..."
    Label(fig[r+1, 1]; text=name, fontsize=9, tellheight=false, halign=:right)
    for (ci, np) in enumerate(n_points)
        add_eiv_learned_panel_sharp!(fig[r+1, ci+1], prob, "eiv", 1, np)
    end
    add_eiv_true_panel_sharp!(fig[r+1, ncols+1], prob)
end

colgap!(fig.layout, 2)
rowgap!(fig.layout, 2)

save(plot_dir() * "/eiv_progression_proxy_sharp_grid.png", fig)
save(plot_dir() * "/eiv_progression_proxy_sharp_grid.pdf", fig)
@info "Done. Saved to plots/eiv_progression_proxy_sharp_grid.png"
