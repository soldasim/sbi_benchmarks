# 4×6 grid of true posteriors — same as plot_all_opt_posteriors.jl but using
# log-proxy variants for problems where they are implemented.
# Currently: BealeProxyProblem, GoldsteinPriceProxyProblem.
...
all_problems = [
    ...
    # 2D-only (log variants where available)
    BealeProxyProblem(),        # log proxy
    BoothProblem(),
    CrossInTrayProblem(),
    DropWaveProblem(),
    EasomProblem(),
    GoldsteinPriceProxyProblem(),  # log proxy
    HimmelblauProblem(),
    HolderTableProblem(),
    LeviN13Problem(),
    MatyasProblem(),
...
@info "Plotting 4×6 grid of true posteriors (with log proxies where available) ..."
...
save(plot_dir() * "/all_opt_posteriors_log_grid.png", fig)
save(plot_dir() * "/all_opt_posteriors_log_grid.pdf", fig)
@info "Done. Saved to $(plot_dir())/all_opt_posteriors_log_grid.png"
