...
const observation_interval = 2.0  # interval length for reference data (entire simulation)
const model_interval = 2.0        # interval length for model outputs
...
function _get_likelihood()
    ...
    return NormalSumLikelihood(; sum_lengths, z_obs, std_obs)
end
...
const A_source = 1.0    # source amplitude (fixed)
...
# Noise parameters
const std_obs = fill(1., n_obs_locs * n_obs_times)  # observation noise
...
function diffusion_pde!(du, u, p, t)
    ...
    for j in 1:ny, i in 1:nx
        ...
        du[idx] = D * (d2u_dx2 + d2u_dy2) - A_adv * (v_x * du_dx + v_y * du_dy) + 100 * source # TODO source const
    end
end
