### MaxVar (LogMaxVar) acquisition with gradient-enhanced GP surrogate.
### For empirically testing Teckentrup convergence bounds with active learning + gradients.
### Uses OptimizationAM() + LogMaxVar() — compare against main_uniform-grads.jl (RandomAM).
