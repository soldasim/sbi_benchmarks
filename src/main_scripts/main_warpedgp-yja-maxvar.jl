### WarpedGaussianProcess (YJ + Affine warping) with LogMaxVar acquisition.
...
function get_output_warpings_yja(problem::AbstractProblem)
    ydim = y_dim(problem)
...
function main(problem::AbstractProblem; data=nothing, iters=100, kwargs...)
    ...
    model = WarpedGaussianProcess(;
        ...
        output_warpings = get_output_warpings_yja(problem),
    )
