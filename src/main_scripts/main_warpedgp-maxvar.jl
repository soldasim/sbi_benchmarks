### WarpedGaussianProcess with LogMaxVar acquisition.
### Uses adaptive YeoJohnson output warping as a learned proxy variable.
### Compare against main_maxvar.jl (plain GP + LogMaxVar).
...
warpedgp_data_dir(problem::AbstractProblem) = "data-warpedgp/" * get_name(problem)
...
function main(problem::AbstractProblem; data=nothing, iters=100, kwargs...)
    ...
    model = WarpedGaussianProcess(;
        ...
        output_warpings = get_output_warpings(problem),
    )
