### MaxVar (LogMaxVar) acquisition without gradients.
### For empirically testing Teckentrup convergence bounds with active learning.
### Uses OptimizationAM() + LogMaxVar() — compare against main_uniform.jl (RandomAM).
