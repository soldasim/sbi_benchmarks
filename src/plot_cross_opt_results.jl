# TV-metric convergence + learned-posterior progression for all 24 cross-polytope 2D problems.
...
const INIT_DATA_CROSS = 3
const ALL_CROSS_PROBLEMS = CrossPolytopeObsProblem.([
    # Original 3
...
for (i, prob) in enumerate(ALL_CROSS_PROBLEMS)
    ...
    plot_result_axis!(fig_tv[r, c], [prob];
        legend = (i == 1),
        metric = :tv,
        plotted_groups = CROSS_GROUPS,
        plot_individual_runs = true,
        compute_slope = false,
    )
end

## ── Data-quality auto-check (NaN / missing iters / unequal budget between maxvar & eiv) ──

function _dq_load_group_scores(problem, run_name; n::Int=20)
    dir = data_dir(problem)
    scores = Vector{Float64}[]
    for idx in 1:n
        fpath = joinpath(dir, "$(run_name)_$(idx)_TVmetric.jld2")
        isfile(fpath) || continue
        try
            push!(scores, load(fpath, "score"))
        catch
        end
    end
    return scores
...
for prob in ALL_CROSS_PROBLEMS
    pname = get_name(prob)
    for grp in CROSS_GROUPS
        dq_register!(DQ_ACC, pname, dq_stat(grp, _dq_load_group_scores(prob, grp)))
    end
end
DQ_NOTES = String[]
for (p, s) in DQ_ACC
...
sort!(DQ_NOTES)
dq_add_banner!(fig_tv, DQ_NOTES; max_shown=8, fontsize=10)

save(plot_dir() * "/cross_all_tv_convergence.png", fig_tv)
save(plot_dir() * "/cross_all_tv_convergence.pdf", fig_tv)
@info "Saved $(plot_dir())/cross_all_tv_convergence.png"

# ─── Posterior progression helpers ────────────────────────────────────────────

function make_cross_bosip(problem, X, Y)
...
function add_cross_panel!(fig_pos, problem, run_name, run_idx, n_points; grid_size=50)
    ax = Axis(fig_pos;
    ...
    try
        ...
        if size(X_full, 2) < n_points
            actual_iters = size(X_full, 2) - INIT_DATA_CROSS
...
function add_cross_true_panel!(fig_pos, problem; grid_size=50)
    ax = Axis(fig_pos;
        xticklabelsvisible=false, yticklabelsvisible=false,
        xticksvisible=false, yticksvisible=false,
        leftspinevisible=false, rightspinevisible=false,
...
# Layout: TV metric | maxvar@iter50 | maxvar@iter100 | maxvar@iter200 |
#                     eiv@iter50   | eiv@iter100    | eiv@iter200    | true

@info "Plotting data progression ..."

n_points_list = [INIT_DATA_CROSS + 50, INIT_DATA_CROSS + 100, INIT_DATA_CROSS + 200]
...
ncols = length(CROSS_GROUPS) * length(n_points_list) + 1   # +1 for true posterior
ch = 180
tv_cw = 150
header_h = 50
col_gap = 2
...
function build_progression_fig(problems)
    ...
    for (gi, grp) in enumerate(CROSS_GROUPS)
    ...
    for (r, prob) in enumerate(problems)
        ...
        for (gi, grp) in enumerate(CROSS_GROUPS)
            ...
            for (si, np) in enumerate(n_points_list)
                ...
                add_cross_panel!(fig[r+1, c], prob, grp, ridx, np)
            end
        end
        add_cross_true_panel!(fig[r+1, ncols + 1], prob)
    end
...
fig = build_progression_fig(ALL_CROSS_PROBLEMS)
save(plot_dir() * "/cross_all_progression_best.png", fig)
save(plot_dir() * "/cross_all_progression_best.pdf", fig)
@info "Done. Saved to $(plot_dir())/cross_all_progression_best.png"
