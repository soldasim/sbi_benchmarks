# 24-row × 5-column progression plot for EIV run 1.
# Rows: problems (same order as all other grids)
# Cols: learned posterior at iter 25 | 50 | 100 | 200* | true posterior
# * "200 iter" column uses all available data; annotated if run terminated early.
...
const INIT_DATA = 3   # initial data points before BO starts
const FULL_N    = INIT_DATA + 200  # expected final data count

# --- Learned posterior panel ---
# use_all: if true, use all available data regardless of n_points (for "200 iter" col)

function add_learned_panel!(fig_pos, problem::AbstractProblem, run_name::String, run_idx::Int, n_points::Int;
                             grid_size=60, use_all=false)
    ax = Axis(fig_pos;
    ...
    # Annotate if run terminated before the expected 200 iterations
    if use_all && N_avail < FULL_N
...
@info "Building 24×5 EIV progression plot ..."
...
for (r, prob) in enumerate(all_problems)
    ...
    # Learned posteriors: cols 2-4 use fixed n_points, col 5 uses all available data
    for (ci, np) in enumerate(n_points)
        is_last = (ci == length(n_points))
        add_learned_panel!(fig[r+1, ci+1], prob, "eiv", 1, np; use_all=is_last)
    end
...
fname = plot_dir() * "/eiv_progression_grid.png"
save(fname, fig)
save(plot_dir() * "/eiv_progression_grid.pdf", fig)
