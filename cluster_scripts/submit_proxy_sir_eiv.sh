#!/bin/bash
# Submit ProxySIRProblem benchmark runs with EIV acquisition (standard GP).
...
for run_idx in 1 2 3 4 5; do
    ...
    sbatch -p cpulong --time=3-00:00:00 --mem=12G --job-name="$job_name" cluster_scripts/run.sh "$pname" eiv "$run_idx" 0 100 nothing
done
