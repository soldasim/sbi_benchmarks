#!/bin/bash
# Submit MaxVar runs for the 12 5D cross-polytope opt-function problems (5 runs each).
...
for pname in "${problems[@]}"; do
    for run_idx in 1 2 3 4 5; do
        ...
        jid=$(sbatch --parsable -p cpu --mem=16G --job-name="$job_name" cluster_scripts/run.sh "$pname" maxvar "$run_idx" 0 200 nothing)
        job_ids+=("$jid")
        count=$((count + 1))
    done
done
