#!/bin/bash
# Submit WarpedGaussianProcess + EIV on the 7 original benchmark problems.
# WarpedGP uses adaptive YeoJohnson output warping as a learned proxy variable.
# Compare against submit_immd.sh (plain GP + EIV) for the same problems.
...
for pname in ABProblem SimpleProblem BananaProblem BimodalProblem ProxySIRProblem; do
    for run_idx in 1 2 3 4 5; do
        job_name="${pname}_warpedgp-eiv_${run_idx}"
        echo "Submitting $job_name ..."
        sbatch -p cpulong --time=3-00:00:00 --mem=12G --job-name="$job_name" \
            cluster_scripts/run.sh "$pname" warpedgp-eiv "$run_idx" 0 100 nothing
    done
done
