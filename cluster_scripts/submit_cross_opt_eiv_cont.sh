#!/bin/bash
# Continue EIV runs for all 24 2D cross-polytope opt-function problems (5 runs each).
# Continues from existing _problem.jld2 files written by the first 24h batch.
...
for pname in "${problems[@]}"; do
    for run_idx in 1 2 3 4 5; do
        ...
        jid=$(sbatch --parsable -p cpu --mem=12G --job-name="$job_name" cluster_scripts/run.sh "$pname" eiv "$run_idx" 1 200 nothing)
        job_ids+=("$jid")
        count=$((count + 1))
    done
done
